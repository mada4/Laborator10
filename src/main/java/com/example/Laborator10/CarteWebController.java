...
import com.example.Laborator10.CarteService;
...
public class CarteWebController {
    ...
    //@RequestMapping(method = RequestMethod.GET, path = "lista-carti")
    ...
    public String operatieAdaugare(@ModelAttribute Carte carte, Model model) {
        if(carte.getIsbn().isBlank() || carte.getTitlu().isBlank() || carte.getAutor().isBlank()) {
            model.addAttribute("str", "Adaugarea nu se realizează dacă nu completaţi toate caracteristicile!");
            model.addAttribute("lista", carteService.findAll());
            return "carti";
        }
        if(carteService.findByIsbn(carte.getIsbn()) != null) {
            model.addAttribute("str", "Cartea cu ISBN-ul " + carte.getIsbn() + " exista deja in repository!");
            model.addAttribute("lista", carteService.findAll());
            return "carti";
        }
    ...
    public String operatieModificare(@ModelAttribute Carte carte, Model model) {
        ...
        if(carte.getTitlu().isBlank() || carte.getAutor().isBlank()) {
            model.addAttribute("str", "Modificarea nu se realizeaza daca nu completati toate caracteristicile!");
            model.addAttribute("lista", carteService.findAll());
            return "carti";
        }
        carteService.update(carte);
        model.addAttribute("str", "Cartea cu ISBN-ul " + carte.getIsbn() + " a fost modificata cu succes!");
        model.addAttribute("lista", carteService.findAll());
    ...
    public String operatieStergere(@ModelAttribute Carte carte, Model model) {
        ...
        model.addAttribute("str", "Stergerea cartii cu ISBN-ul " + carte.getIsbn() + " a fost realizata cu succes!");
        model.addAttribute("lista", carteService.findAll());
